...
class ConvBlock1D(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int, kernel_size: int = 15) -> None:
        super().__init__()
        pad = kernel_size // 2
...
class UpBlock1D(nn.Module):
    ...
    def __init__(self, in_ch: int, skip_ch: int, out_ch: int, kernel_size: int = 15) -> None:
        super().__init__()
        self.skip_reduce = nn.Conv1d(skip_ch, max(skip_ch // 2, 1), kernel_size=1, bias=False)
        self.skip_bn = nn.BatchNorm1d(max(skip_ch // 2, 1))
        self.skip_relu = nn.ReLU(inplace=True)

        self.skip_out_ch = max(skip_ch // 2, 1)
        self.conv = ConvBlock1D(in_ch + self.skip_out_ch, out_ch, kernel_size=kernel_size)

    def forward(self, x: torch.Tensor, skip: torch.Tensor) -> torch.Tensor:
        x = F.interpolate(x, size=skip.shape[-1], mode="linear", align_corners=False)
        skip = self.skip_reduce(skip)
        skip = self.skip_bn(skip)
        skip = self.skip_relu(skip)
...
class DilatedResBlock1D(nn.Module):
    """Residual dilated block for long-range temporal context."""

    def __init__(self, channels: int, dilation: int, kernel_size: int = 15) -> None:
        super().__init__()
...
class TemporalContextModule(nn.Module):
    """
    Multi-dilation bottleneck to emphasize long-range features.
    """

    def __init__(self, channels: int, kernel_size: int = 15) -> None:
        super().__init__()
        self.blocks = nn.Sequential(
            DilatedResBlock1D(channels, dilation=1, kernel_size=kernel_size),
            DilatedResBlock1D(channels, dilation=2, kernel_size=kernel_size),
            DilatedResBlock1D(channels, dilation=4, kernel_size=kernel_size),
            DilatedResBlock1D(channels, dilation=8, kernel_size=kernel_size),
            DilatedResBlock1D(channels, dilation=16, kernel_size=kernel_size),
            DilatedResBlock1D(channels, dilation=32, kernel_size=kernel_size),
        )
...
# ============================================================
# Long-context 1D U-Net
# ============================================================

class UNet1DLongContext(nn.Module):
    """
    1D U-Net with dilated bottleneck context for longer temporal structure.
    Input:  (B, 2, T)
    Output: (B, 1, T)
    """
    ...
    def __init__(
        ...
        self.context = TemporalContextModule(c5, kernel_size=kernel_size)
...
class BCEDiceSmoothLoss(nn.Module):
    """
    BCE with logits + Dice + temporal smoothness penalty.
    This helps reduce fragmented predictions.
    """
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        return bce + self.dice_weight * dice + self.smooth_weight * smooth
...
class SwimBoutUNetDetector:
    ...
    def fit(
        ...
        train_loader, val_loader = self._make_dataloaders(x, y, normalize=normalize)
    ...
    def predict_label(
        ...
        probs = self.predict_proba(x, batch_size=batch_size, normalize=normalize)
    ...
    def predict(
        ...
        x = np.vstack((res["fltCh0"], res["fltCh1"])).astype(np.float32)
        if return_probs:
            return self.predict_proba(x, batch_size=batch_size, normalize=normalize)
        else:
            return self.predict_label(x, threshold=threshold, batch_size=batch_size, normalize=normalize)
...
if __name__ == "__main__":
    ...
    detector.save(r"D:\EnData\10161\S1\swim_detector_long_context.pt")

    probs = detector.predict(
    ...
    plt.plot(probs, label="Predicted Probability", linewidth=0.5)
    plt.legend()
    plt.tight_layout()
    plt.show()